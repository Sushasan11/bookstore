...
1. `test_list_users_paginated` — GET /admin/users returns items list with total_count, page, per_page, total_pages. Create 3 users + 1 admin, verify total_count >= 4, items is a list of objects with id/email/role/is_active/created_at.
...
12. `test_deactivate_blocks_access_token` — After deactivation, using the user's existing access token on a protected endpoint (e.g. GET /cart) returns 403 "Account deactivated" (IF cart route uses ActiveUser; otherwise test against an admin endpoint). NOTE: If only admin routes use ActiveUser, test with a route that does use it. At minimum, verify the is_active check works by attempting to access an endpoint that uses the get_active_user dependency.
