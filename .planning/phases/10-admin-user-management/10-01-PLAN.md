...
must_haves:
  truths:
    ...
    - "Deactivated user is immediately locked out: access token rejected via DB is_active check, login blocked, refresh already blocked"
...
Output: Working admin user management endpoints at /admin/users with immediate is_active lockout on all protected routes.
</objective>
...
**Scope note on ActiveUser for existing routes:** For Phase 10, apply ActiveUser only to admin routes (via the AdminUser chain). Do NOT change existing cart/orders/wishlist routes from CurrentUser to ActiveUser in this plan — that is a broader change that can be done in a follow-up if needed. The CONTEXT.md "immediate lockout" is primarily enforced via the login block + refresh block + admin endpoint chain.
  </action>
  <verify>
...
class AdminUserResponse(BaseModel):
    """Admin view of a user account. Omits full_name — column does not exist on User model."""
    id: int
    email: str
    role: str
    is_active: bool
...
Per CONTEXT.md decision: includes id, email, role, is_active, created_at. Omits full_name (User model has no such column — research confirmed this is aspirational, not implemented).
...
```
  ...
  <done>
    ...
    - AdminUserResponse schema has id, email, role, is_active, created_at (no full_name)
...
3. Lint: `cd D:/Python/claude-test && ruff check app/admin/ app/core/deps.py app/users/repository.py app/users/service.py`
4. Format: `cd D:/Python/claude-test && ruff format --check app/admin/ app/core/deps.py app/users/repository.py app/users/service.py`
</verification>
...
- Deactivated user's access token is rejected on protected endpoints (403 from ActiveUser dep)
